...
void InteractionCards(sf::RenderWindow& window, float deltaTime, int& count,
	...
	if((sf::Keyboard::isKeyPressed(sf::Keyboard::Left) || position.x < (scrX / 2 - 210)) && 
		startupCard.getX() > 325){
		startupCard.moveSprite(-deltaTime, 0);
	}
	else if((sf::Keyboard::isKeyPressed(sf::Keyboard::Right) || position.x > (scrX / 2 + 210)) 
		&& startupCard.getX() < 625) {
		startupCard.moveSprite(deltaTime, 0);
	}
